...
function fillData(orden, noOrden, fromto) {
    ...
    const html = `
        ...
        <p><strong>No. de venta:</strong> ${orden.id_venta}</p>
        <p><strong>Producto:</strong> ${orden.categoria} TAM.${orden.size}</p>
        <p><strong>Cantidad inicial:</strong> ${orden.cantidad_inicial}
        <p><strong>Cantidad buenos:</strong></p> <input type="number" id="orden_cantidad_buenos" step="1" min="1" value="${orden.cantidad_buenos}">
        <p><strong>Cantidad rotos:</strong></p><input type="number" id="orden_cantidad_rotos" step="1" min="1" value="${orden.cantidad_rotos}"> 
        <p><strong>Cantidad deformes:</strong></p> <input type="number" id="orden_cantidad_deformes" step="1" min="1" value="${orden.cantidad_deformes}">
        <p><strong>Fecha de entrega:</strong></p> <input type="number" id="orden_fecha_entrega" step="1" min="1" value="${formattedDate}">
    `;
...
document.getElementById('cancel-edit').addEventListener('click', () => {
    ...
    const cardElem = document.querySelector('.kanban-card.editando'); // o guarda una referencia global
