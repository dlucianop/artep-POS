...
function fillDetails(idOrden) {
    // 1) localizar la orden
    // si usas id_orden numérico, extrae el número del string "card-123"
    const orden = ordenes.find(o => o.id_orden === idOrden);
    if (!orden) return console.error('Orden no encontrada:', idOrden);
  
    // 2) construir el HTML de detalles
    const html = `
      <p><strong>Origen:</strong> ${orden.origen}</p>
      <p><strong>ID Venta:</strong> ${orden.id_venta}</p>
    ...
    // 3) inyectar en el contenedor
    document.getElementById('detail-content').innerHTML = html;
  
}
  
  // cerrar con el botón automático
